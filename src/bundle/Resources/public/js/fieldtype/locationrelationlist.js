(function (global, doc, ibexa, React, ReactDOM, Translator) {
    ...
    const SELECTOR_FIELD_MULTIPLE = '.ibexa-field-edit--locationrelationlist';
...
console.log(items);
                itemNode.setAttribute('data-location-id', escapeHTML(item.id));
        ...
        const openUDW = (event) => {
            ...
            const title =
                limit === 1
                    ? Translator.trans(
                          /*@Desc("Select a Location item")*/ 'locationrelationlist.title.single',
                          {},
                          'universal_discovery_widget',
                      )
                    : Translator.trans(
                          /*@Desc("Select Location item(s)")*/ 'locationrelationlist.title.multi',
                          {},
                          'universal_discovery_widget',
                      );
        ...
        const excludeDuplicatedItems = (items) => {
            selectedItemsMap = items.reduce((total, item) => ({ ...total, [item.id]: item }), selectedItemsMap);
        ...
        const removeItem = (event) => {
            ...
            const locationId = parseInt(row.dataset.locationId, 10);

            row.remove();

            selectedItems = selectedItems.filter((item) => locationId !== item);
        ...
        const updateSelectedItemsOrder = (event) => {
            ...
            selectedItems = inputs.map((item) => parseInt(item.row.dataset.locationId, 10));
            updateInputValue(selectedItems);
        };
        let selectedItems = [...fieldContainer.querySelectorAll(SELECTOR_ROW)].map((row) => parseInt(row.dataset.locationId, 10));
